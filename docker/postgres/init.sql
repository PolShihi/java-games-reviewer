...
('CD Projekt Red', 'Adam Kicinski', 2002, (SELECT id FROM company_types WHERE name = 'Hybrid' LIMIT 1)),
...
('The Witcher 3: Wild Hunt', 2015, 'Geralt of Rivia searches for Ciri.',
  (SELECT id FROM production_companies WHERE name = 'CD Projekt Red' LIMIT 1),
  (SELECT id FROM production_companies WHERE name = 'CD Projekt Red' LIMIT 1)),
('Grand Theft Auto V', 2013, 'Three criminals in Los Santos.',
  (SELECT id FROM production_companies WHERE name = 'Rockstar Games' LIMIT 1),
  (SELECT id FROM production_companies WHERE name = 'Rockstar Games' LIMIT 1)),
('Elden Ring', 2022, 'Tarnished in the Lands Between.',
  (SELECT id FROM production_companies WHERE name = 'FromSoftware' LIMIT 1),
  (SELECT id FROM production_companies WHERE name = 'Bandai Namco' LIMIT 1));
...
((SELECT id FROM games WHERE title = 'The Witcher 3: Wild Hunt'),
 (SELECT id FROM system_requirement_types WHERE name = 'medium'),
 35, 8, 3.4, 4.0, 2),
...
((SELECT id FROM games WHERE title = 'The Witcher 3: Wild Hunt'),
 (SELECT id FROM media_outlets WHERE name = 'IGN'),
 93, 'An amazing RPG experience.'),
((SELECT id FROM games WHERE title = 'The Witcher 3: Wild Hunt'),
 (SELECT id FROM media_outlets WHERE name = 'PC Gamer'),
 92, 'One of the best PC games ever.'),
((SELECT id FROM games WHERE title = 'Grand Theft Auto V'),
 (SELECT id FROM media_outlets WHERE name = 'IGN'),
 100, 'A masterpiece.'),
((SELECT id FROM games WHERE title = 'Elden Ring'),
 (SELECT id FROM media_outlets WHERE name = 'GameSpot'),
 100, 'FromSoftware best work.'),
((SELECT id FROM games WHERE title = 'Elden Ring'),
 (SELECT id FROM media_outlets WHERE name = 'IGN'),
 100, 'Simply incredible.');
